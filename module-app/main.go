...
func fetchLatLong(city string) (*LatLong, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error making a request to the Geo API: %w", err)
	}
...
func getLatLong(db *sqlx.DB, name string) (*LatLong, error) {
	var latLong *LatLong
	err := db.Get(&latLong, "SELECT lat, long FROM cities WHERE name = $1")
	if err == nil {
		return latLong, nil
	}
...
func extractWeatherData(city string, rawWeather string) (WeatherDisplay, error) {
	...
	for i, t := range weatherResponse.Hourly.Time {
		date, err := time.Parse(time.RFC3339, t+":00+10:00")
		if err != nil {
...
func main() {
	...
	r.GET("/weather", func(c *gin.Context) {
		...
		// grab the weather form the latLong
		weather, err := getWeather(*latlong)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
		// display the weather data in an html template
